...
function UTChat:PostMessage( utxt )
	...
	for ix,m in ipairs(self.Messages) do
		m.MessageID = ix
		local pos = Vector2(70,(Render.Height*self.position)-((ix-1)/10*160))
		if ix == 1 then
			m.position = pos
		else
...
function UTChat:OnPlayerChat( args )
	...
	utxt = UText(args.player:GetName() .. ": " .. args.text,Vector2(70,(Render.Height*self.position)))
...
function UTChat:PrintChat( args )
	...
	local utxt = UText(args.text,Vector2(70,(Render.Height*self.position + 0.05)),duration,color,{alpha=alpha, nochathandling = args.noparse or false})
