...
function ListaPlus:Render(posicao, largura, margem)
	...
	for index = self.itemTopo, fim, 1 do
		...
		if (item) then
			...
			-- Texto
			Render:DrawText(posicao + Vector2(margem, altura * linha + altura / 2 - Render:GetTextHeight(tostring(item.texto), size) / 2), tostring(item.texto), corTexto, size)
			-- TextoSecundario
			Render:DrawText(posicao + Vector2(largura - margem - Render:GetTextWidth(tostring(item.textoSecundario), size), altura * linha + altura / 2 - Render:GetTextHeight(tostring(item.textoSecundario), size) / 2), tostring(item.textoSecundario), corTexto, size)
		end
	end
